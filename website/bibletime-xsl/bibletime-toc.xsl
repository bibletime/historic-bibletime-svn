...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:param name="nav.text.revisionflag.added"></xsl:param>
	<xsl:param name="nav.text.revisionflag.changed"></xsl:param>
	...
	<xsl:template match="tocentry">
		...
		<xsl:variable name="use.toc.expand.depth">
			<xsl:variable name="config-param"
				select="ancestor::autolayout/config[@param='toc.expand.depth']/@value" />
			<xsl:choose>
				<!-- toc.expand.depth attribute is not in DTD -->
				<xsl:when test="ancestor::toc/@toc.expand.depth">
					<xsl:value-of
						select="ancestor::toc/@toc.expand.depth" />
				</xsl:when>
		...
		<li>
			...
			<xsl:choose>
				...
				<xsl:otherwise><!-- Not the current page -->
					<xsl:choose>
			...
			<xsl:if test="$is.open != 0"><!-- Item with child items -->
				<xsl:choose>
