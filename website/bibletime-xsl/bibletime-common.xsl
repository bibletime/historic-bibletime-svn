...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="output-flags">
		...
		<xsl:choose>
			<xsl:when
				test="string-length(normalize-space( substring-before($langs, ' ') )) &gt; 0">
				<xsl:call-template name="print-flag">
					<xsl:with-param name="lang"
						select="substring-before(normalize-space($langs), ' ')" />
					<xsl:with-param name="htmlfilename"
						select="$htmlfilename" />
				</xsl:call-template>
			</xsl:when>
			<xsl:when
				test="string-length(normalize-space($langs)) &gt; 0">
				<xsl:call-template name="print-flag">
					<xsl:with-param name="lang"
						select="normalize-space($langs)" />
					<xsl:with-param name="htmlfilename"
						select="$htmlfilename" />
				</xsl:call-template>
			</xsl:when>
		...
		<xsl:if
			test="string-length(normalize-space(substring-after($langs, ' '))) &gt; 0">
			<xsl:call-template name="output-flags">
				<xsl:with-param name="langs"
					select="substring-after($langs, ' ')" />
				<xsl:with-param name="htmlfilename"
					select="$htmlfilename" />
			</xsl:call-template>
		</xsl:if>
	...
	<xsl:template match="id('devotional')" mode="intro">
		<div id="devotional">
			<h1>
				<xsl:value-of
					select="./child::section[position()=1]/title" />
			</h1>

			<xsl:apply-templates
				select="./child::section[position()=1]/para[position()=1]" />

			<xsl:if
				test="count(./child::section[position()=1]/para) &gt; 1">
				<p>
					[
