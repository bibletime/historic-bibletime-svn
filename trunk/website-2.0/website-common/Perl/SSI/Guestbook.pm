...
sub show_addItem() {
	...
	#Insert the robot check calc task
	my $min = 1;
	my $max = 6;
	
	my $random1 = int( rand($max)) + $min;
	my $random2 = int( rand($max)) + $min;
#	my $result = $random1*$random2;

	$ret .= $q->Tr( $q->td({-colspan=>'2'}, "To prevent spam please enter the result of this small calculation:"));
	$ret .= $q->Tr( $q->td({-colspan=>'2'}), "$random1 * $random2 = ", $q->textfield({-name=>'result', -override=>'1', -value=>'', -size=>'5'}));
	
	$ret .= $q->Tr( $q->td({-colspan=>'2', -align=>'center'}, $q->submit( $self->i18n_safe("Add your comments ...")) ));
	$ret .= $q->Tr( $q->td({-colspan=>'2', -height=>'20'})); # space

	$ret .= $q->end_table;
	$ret .= $q->hidden("number1", $random1);	
	$ret .= $q->hidden("number2", $random2);	
	$ret .= $q->end_form;
...
sub addItem {
	...
	my $number1 = $q->param('number1');	
	my $number2 = $q->param('number2');	

	#emails will be made unreadable for robots in the output script
	$comments =~ s/<.+?>// if ($comments); # remove HTML tags from comments

	if ($result != ($number1*$number2)) {
		return $q->div({-class=>'warning'}, $q->h3($self->i18n("BibleTime Guestbook")) . $self->i18n("You have to enter the right solution to the math task to write to the guesbook."));
	};

	if (!($name && $comments)) { #we must have these two values!
		return $q->div({-class=>'warning'},$q->h3($self->i18n("BibleTime Guestbook")) . $self->i18n("Please enter your name and the comment! The entry was not added to the guestbook!"));
	};
	...
	$sth->execute (
		...
		$comments,
		$today
	) || return $q->div({-class=>'warning'},$q->h3($self->i18n("BibleTime Guestbook")) . $self->i18n("Your comments couldn't be written into the guestbook. The command to write the data to disk failed:") . $q->br() . $q->i(DBI->errstr));
