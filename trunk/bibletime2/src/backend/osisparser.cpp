...
QString parse_reflist(const QString& reflist)
{
	...
	toplevelSeparator.setPattern(reflist.contains(":") ? ";|," : ";");

	//Start by splitting up the toplevel ranges which are independent of each other
	// e.g. Gen 1,2-3:5; Mt 3,2 -> "Gen 1,2-3:5", "Mt 3,2"
	...
	QString contextBook = "Gen";
	QString currentBook;
	unsigned int contextChapter = 0;
	unsigned int currentChapter = 0;
	unsigned int currentVerse = 0;
	bool verseDefined = false;
	...
	for (toplevelRange = toplevelRanges.constBegin(); toplevelRange != toplevelRanges.constEnd(); ++toplevelRange)
	{
		...
		for (sublevelRange = sublevelRanges.constBegin(); sublevelRange != sublevelRanges.constEnd(); ++sublevelRange)
		{
			...
			{
				//qDebug() << "sl: " << *sublevelRange << " 1 " << rx.cap(1) << " 2 " << rx.cap(2) << " 3 " << rx.cap(3) << " vd " << verseDefined;

				if (!rx.cap(1).isEmpty()) //book found?
					contextBook = currentBook = rx.cap(1);
				else
					currentBook = contextBook;
				
				if (!rx.cap(2).isEmpty()) //element 2 found, could be chapter or verse
				{ 
					if (!rx.cap(3).isEmpty()) //element 3 found as well
					{
						contextChapter = currentChapter = rx.cap(2).toUInt();
						currentVerse = rx.cap(3).toUInt();
						verseDefined = true;
					}
					else //no element 3
					{
						if (verseDefined) //verse was defined before, use element 2 as verse
						{
							currentChapter = contextChapter;
							currentVerse = rx.cap(2).toUInt();
							verseDefined = true;
						}
						else //use element 2 as chapter, no verse available
						{
							contextChapter = currentChapter = rx.cap(2).toUInt();
							currentVerse = 0;
							verseDefined = false;
				...
				else //element 2 and 3 empty
				{
					currentVerse = currentChapter = 0;
					verseDefined = false;
				}

				//output
				if (sublevelRange != sublevelRanges.constBegin())
					result.append("-");
				
				result.append(currentBook);
				
				if (currentChapter > 0)
					result.append('.').append(QString::number(currentChapter));
				
				if (currentVerse > 0)
					result.append('.').append(QString::number(currentVerse));
				
			}
		}//for-loop sublevelRanges
		result.append("|"); //Separate toplevelRanges
	}//for-loop toplevelRanges
	return result;
}
...
/* Code by David White, store for now
